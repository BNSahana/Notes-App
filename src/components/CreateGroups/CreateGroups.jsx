...
const CreateGroups = (props) => {
  ...
  const renderMobileCreateGroups = () => (
    <>
      {" "}
  ...
  const renderDesktopCreateGroup = () => (
    <>
      <div className={styles.desktop__modal__background}>
        <div ref={modalRef} className={styles.desktop__modal__container}>
          ...
          <input
            ...
            onChange={handleChange}
          />
          <label className={styles.desktop__modal__color}>Choose Colour</label>
          {Object.entries(colorOptions).map(([key, color]) => (
            <button
              className={`${styles.colorButton}  ${
                formData.color === color ? "selected" : ""
  ...
  return screenSize.width < 990
    ? renderMobileCreateGroups()
    : renderDesktopCreateGroup();
};
